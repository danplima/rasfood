...
import br.om.rasmoo.restaurante.vo.ClienteVo;
...
public class EnderecoDao {
    ...
    public List<ClienteVo> consultarClientes(final String estado, final String cidade, final String rua) {
        String jpql = "SELECT new br.om.rasmoo.restaurante.vo.ClienteVo(e.cliente.cpf, e.cliente.nome) FROM Endereco e " +
                "WHERE UPPER(e.estado) = UPPER(:estado) AND " +
                "UPPER(e.cidade) = UPPER(:cidade) AND " +
                "UPPER(e.rua) = UPPER(:rua)";
        return this.entityManager.createQuery(jpql,ClienteVo.class)
                .setParameter("estado", estado)
                .setParameter("cidade",cidade)
                .setParameter("rua", rua).getResultList();
    }
